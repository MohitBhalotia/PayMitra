...
const AdminDashboard = () => {
  ...
  const handleReleasePayment = async (projectId, milestoneId) => {
    try {
      await paymentService.releasePayment(projectId, milestoneId);
  ...
  const filteredProjects = projects.filter(project => {
    if (selectedTab === 'pending') {
      return project.milestones.some(m => m.status === 'approved' && m.status !== 'paid');
    }
              ...
              {filteredProjects.map((project) =>
                project.milestones.map((milestone) => (
                  <tr key={`${project.id}-${milestone.id}`}>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span
                        className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${milestone.status === 'approved'
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                      {milestone.status === 'approved' && !milestone.paymentId && (
                        <button
                          onClick={() => handleReleasePayment(project.id, milestone.id)}
                      ...
                      {milestone.status === 'pending' && (
                        <button
                          onClick={() => handleRefundPayment(project.id)}
